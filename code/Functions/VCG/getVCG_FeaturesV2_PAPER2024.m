function [ S ] = getVCG_FeaturesV2_PAPER2024( vcg, debug_flag, str_class )
    ...
    M = [R2x, D1x, D2x; ...
     ...
     if debug_flag
         if idx == 1
             getValues(x, [y, z], true, str_class);
         elseif idx == 2
             getValues(y, [x, z], true, str_class);
         elseif idx == 3             
             getValues(z, [x, y], true, str_class);
         end
     end
...
function [R2, D1, D2] = getValues(y, X, debug_flag, str_class)
    ...
    if debug_flag == true
        plot_vcg(y, X(:, 2), X(:, 3), b, str_class);
    end    
    
end

function plot_vcg(x, y, z, b, str_class)

    t1 = [str_class, ' - VCG (3D View)'];
    t2 = [str_class, ' - VCG (XY View)'];
    t3 = [str_class, ' - VCG (XZ View)'];
    ...
    title(t2, 'Interpreter', 'Latex', 'FontSize', 14); % x, y
    
    subplot (1, 3, 3);
    plot_aux(x, y, z, b);
    az = 90; el = 0; view([0 -1 0]);
    title(t3, 'Interpreter', 'Latex', 'FontSize', 14); % x, z
    
    set(gcf, 'Position', [100, 100, 1200, 350]);

end

function plot_vcg_qrs(x, y, z, b)
    
    figure;
    subplot (1, 2, 1);
    plot_aux(x, y, z, b);
    title('VCG(QRS) (Vista 1)', 'Interpreter', 'Latex', 'FontSize', 14);
    view([136.01 14.36]);
    
    subplot (1, 2, 2);
    plot_aux(x, y, z, b); 
    az = 0; el = 0; view([0 0 1]);
    title('(Vista 2)', 'Interpreter', 'Latex', 'FontSize', 14); % x, y
...
function plot_aux(x, y, z, b)
    ...
    fs = 14;
    xlabel('\emph{X}', 'Interpreter', 'Latex', 'FontSize', fs);
    ylabel('\emph{Y}', 'Interpreter', 'Latex', 'FontSize', fs);
    zlabel('\emph{Z}', 'Interpreter', 'Latex', 'FontSize', fs);
    view(50, 10)
    grid
    hold off
